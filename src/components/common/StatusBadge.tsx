...
export const StatusBadge: React.FC<StatusBadgeProps> = ({ 
  ...
  if (norm === 'ACTIVE' || norm === 'LINKED' || norm === 'RESOLVED' || norm === 'PROCESSED' || norm === 'VERIFIED') {
    colors = 'bg-emerald-950/50 text-emerald-400 border-emerald-500/50 shadow-[0_0_6px_rgba(52,211,153,0.2)]';
  } else if (norm === 'FLAGGED' || norm === 'CRITICAL' || norm === 'HIGH' || norm === 'SUSPICIOUS') {
    colors = 'bg-red-950/50 text-red-400 border-red-500/60 shadow-[0_0_6px_rgba(239,68,68,0.2)]';
  } else if (norm === 'NEEDS_VERIFICATION' || norm === 'REQUIRES REVIEW' || norm === 'UNCONFIRMED' || norm === 'IN_REVIEW' || norm === 'AWAITING_REVIEW') {
    colors = 'bg-amber-950/60 text-amber-300 border-amber-500/60 shadow-[0_0_6px_rgba(245,158,11,0.2)]';
  } else if (norm === 'NEW' || norm === 'INDEXING' || norm === 'PENDING') {
