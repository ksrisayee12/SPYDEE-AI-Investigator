...
import { CASE_METADATA } from '../../data/mockCaseData';
...
export const TerminalShell: React.FC<TerminalShellProps> = ({
  ...
  const navItems: {
    section: string;
    items: { id: NavTab; label: string; icon: React.ComponentType<{ className?: string }>; count?: number }[];
  }[] = [
    {
      section: '// CASE CONSOLE',
      items: [
        { id: 'overview', label: 'CASE OVERVIEW', icon: LayoutDashboard },
        { id: 'evidence', label: 'EVIDENCE & FILES', icon: FileText, count: 6 },
        { id: 'entities', label: 'ENTITIES', icon: Users, count: 27 },
        { id: 'graph', label: 'GRAPH', icon: Network, count: 84 },
        { id: 'map', label: 'MAP', icon: MapPin },
        { id: 'timeline', label: 'TIMELINE', icon: Clock, count: 9 }
      ]
    },
    {
      ...
      items: [
        ...
        { id: 'hypotheses', label: 'HYPOTHESES', icon: GitFork, count: 6 },
        { id: 'contradictions', label: 'CONTRADICTIONS', icon: AlertTriangle, count: 2 },
        { id: 'gaps', label: 'INFORMATION GAPS', icon: HelpCircle, count: 6 },
        { id: 'leads', label: 'LEADS & ACTIONS', icon: Compass, count: 4 }
      ]
    },
    {
      section: '// TOOLS',
      items: [
        { id: 'ai_investigator', label: 'AI INVESTIGATOR', icon: Cpu },
  ...
  return (
    <div className="relative w-screen h-screen flex flex-col bg-[#070b07] text-[#f59e0b] overflow-hidden select-none font-mono">
      {/* Scanline overlay (software-only CRT texture) */}
          ...
          <footer className="h-6 bg-[#0a0f0a] border-t border-amber-500/30 px-3 flex items-center justify-between text-[10px] text-amber-500/70 shrink-0 select-none">
            <div className="flex items-center gap-4">
              ...
              <span className="hidden md:inline-block border-l border-amber-500/30 pl-3">
                GRAPH NODES: {CASE_METADATA.metrics.entities} // EDGES: {CASE_METADATA.metrics.relationships}
              </span>
            </div>
